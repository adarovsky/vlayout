...
export class Layout
    ...
    private parseTypePair(): boolean {
        ...
        if ((name = this.matchIdentifier())) {
            this.matchOrFail(':');
            if (
                !this.parseEnum(name.content) &&
                !this.parseList(name.content)
            ) {
    ...
    private compareTypes(
        context: string,
        item1: ListDefinitionItem | TypeDefinition,
        item2: ListDefinitionItem | TypeDefinition
    ) {
    ...
    private parseInput(keyPath: string): boolean {
        ...
        if (name) {
            ...
            if (this.match('{')) {
                ...
                this.props.engine.inputs.registerInputReference(
                    kp,
                    typeName!.content,
                    name.line,
                    name.column
    ...
    private parseProperties(): void {
        if (this.match('properties')) {
            this.properties = new CompoundPropertyDeclaration(
                '',
    ...
    private parseProperty(): PropertyDeclaration | null {
        ...
        if (name) {
            if (this.match('{')) {
                ...
                return new ExpressionPropertyDeclaration(
                    name.content,
                    e!,
                    name.line,
                    name.column
    ...
    private parseConditional(): Expression | null {
        ...
        if (cond && this.match('?')) {
            ...
            if (!right)
                this.raiseError(`expression expected after ${cond} ? ${left}`);
    ...
    private parseLogicalOr(): Expression | null {
        ...
        while (this.match('||')) {
            ...
            if (!nextOr) this.raiseError(`Expression expected after ${node}`);
    ...
    private parseLogicalAnd(): Expression | null {
        ...
        while (this.match('&&')) {
            ...
            if (!nextAnd) this.raiseError(`Expression expected after ${node}`);
    ...
    private parseEqual(): Expression | null {
        ...
        if (token) {
            ...
            if (!right)
                this.raiseError(
                    `Expression expected after ${left} ${token.content}`
                );
            return token.content === '=='
                ? new EqualExp(left!, right!)
    ...
    private parseCompare(): Expression | null {
        ...
        if (token) {
            ...
            if (token.content === '>') return new More(left!, right!);
            else if (token.content === '>') return new MoreEqual(left!, right!);
            else if (token.content === '<') return new Less(left!, right!);
                ...
                this.raiseError(
                    `invalid operation ${token.content} after ${left}`
                );
            }
        }
    ...
    private parseNegate(): Expression | null {
        if (this.match('!') || this.match('-')) {
            ...
            if (!unit) this.raiseError('Expression expected after !');
    ...
    private parseAlternative(): Expression | null {
        ...
        while ((token = this.match('|'))) {
            ...
            if (!item)
                this.raiseError(
                    `Expression expected after ${item} ${token.content}`
                );
            array.push(item!);
        }

        if (array.length === 1) return array[0];
        else return new Alternative(array);
    }
        ...
        if (this.match('switch')) {
            ...
            while (this.match('case')) {
                ...
                if (d.length === 1 && isFirstWildcard) {
                    ...
                    this.raiseError(
                        `number of items in matcher ${d.length} does not match sources: ${sources.length}`
                    );
                }

                if (hasParen) this.matchOrFail(')');
                ...
                if (!result) {
                    this.raiseError(
                        'expression expected when parsing case _ =>'
                    );
                }
                let matcher = new SwitchMatcher(
                    caseLex.line,
                    caseLex.column,
                    d,
                    result!
    ...
    private parseReference(): Expression | null {
        ...
        if (this.match('.')) {
            ...
            while (this.match('.')) {
                ...
                if (!unit) {
                    this.raiseError(
                        `Identifier expected after ${kp.join('.')}`
                    );
                }
        ...
        if (name) {
            ...
            if (name.content === 'model') {
                ...
                if (!ref) {
                    ...
                    this.raiseError(
                        `only input references supported in models`
                    );
                }
            ...
            if (this.match(':')) {
                ...
                if (exp) {
                    ...
                    if (prop) {
                        ...
                        this.raiseError(
                            `property ${name.content} is not found in ${list}`
                        );
                    }
                }
            } else {
            ...
            if (this.match('{')) {
                ...
                if (view) {
                    ...
                    if (callback) {
                        ...
                        const handler = this.engine.listTextFieldForKey(
                            name.content
                        );
                        if (handler) {
                            view = new ListTextField(
                                name.content,
                                handler.onChange,
                                handler.onEnter
                            );
                ...
                if (!exp) {
                    this.raiseError(
                        `expression expected after ${name.content} :`
                    );
                }
                const prop = container.property(name.content);
                if (!prop) {
                    this.raiseError(
                        `property ${name.content} not found in ${container}`
                    );
                }
                prop.value = exp!;
                prop.line = name.line;
    ...
    private parseViewContents(view: View): boolean {
        ...
        if (name) {
            ...
            if (!prop) {
                this.raiseError(
                    `property ${name.content} not found in ${view}`
                );
            }
            prop.value = exp!;
            prop.line = name.line;
    ...
    private parsePropertyContents(
        ...
        if (prop.value) {
            this.raiseError(
                `property  ${name} in view ${view} already has value ${prop.value}`
            );
        }
        prop.value = exp!;
    ...
    variableForKeyPath(keyPath: string): Expression | null {
        if (keyPath == 'viewport.width') {
            return new BuiltinProperty(
                keyPath,
                this.engine.numberType(),
                this.currentSize.pipe(pluck('width'))
            );
        } else if (keyPath == 'viewport.height') {
            return new BuiltinProperty(
                keyPath,
                this.engine.numberType(),
                this.currentSize.pipe(pluck('height'))
            );
        }
        ...
        for (let k of kp) {
            ...
            if (!(ret instanceof CompoundPropertyDeclaration)) {
                throw new LinkError(
                    line,
                    column,
                    `property ${ret} does not have member ${k}`
                );
            }
    ...
    functionFor(
        ...
        const f = this.functions
            ? this.functions.functionFor(name, parameters)
            : null;
        if (f) return f;
    ...
    functionsLoose(
        ...
        const arr = this.functions
            ? this.functions.functionsLoose(name, parametersCount)
            : [];
    ...
    private raiseError(message: string): never {
        throw new ParseError(
            this._lexer.currentLine,
            this._lexer.currentColumn,
            message
        );
    }
