...
export class Bindings {
    ...
    registerSlot(name: string, type: string): void {
        if (type === 'listButton' && this.engine.listButtonForKey(name)) return;

        if (type === 'listTextField' && this.engine.listTextFieldForKey(name))
            return;

        if (type === 'listView' && this.engine.listViewForKey(name)) return;
        ...
        if (type === 'button' && !(v instanceof Button)) {
            throw new Error(
                `view ${name} is not a button, but ${v.viewType()}`
            );
        }

        if (type === 'textField' && !(v instanceof TextField)) {
            throw new Error(
                `view ${name} is not a text field, but ${v.viewType()}`
            );
        }
    }
