...
function WowComp(
    props: Wow & { topOffset: number; image: (s: string) => string }
) {
    return (
        <div
            className={'wow_row'}
...
export class WowField extends Component<WowFieldProps, WowFieldState> {
    ...
    componentDidMount(): void {
        this.subscription.add(
            this.props.shoot.subscribe(type => this.shoot(type))
        );
        if (this.objectRef.current) {
            this.subscription.add(
                resizeObserver(this.objectRef.current).subscribe(size => {
                    // console.log(size);
                    this.setState({ size });
                })
            );
    ...
    shoot(type: string) {
        ...
        this.setState(s => ({ ...s, wows: s.wows.concat([wow]) }));
        this.subscription.add(
    ...
    render() {
        return (
            <div style={{ width: '100%', height: '100%' }} ref={this.objectRef}>
                {this.state.wows.map(w => (
                    <WowComp
                        id={w.id}
                        key={w.id}
                        type={w.type}
...
export class LayoutWowField extends ReactView<
    WowFieldProps & ReactViewProps,
    ReactViewState & WowFieldState
> {
    render() {
        const extra = _.pick(this.state, 'id');
        return (
            <div
                className={this.state.className || 'vlayout_wowField'}
                style={this.style()}
                {...extra}
            >
                <WowField {...this.props} />
